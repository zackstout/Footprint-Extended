myApp.service('csvService', function ($http, $location, UserService) {
  ...
  vm.parseData = function (data) {
    ...
    // var res = vm.valuesToArray(csv);
    // wait what is this doing???
    // Hmmmm everything seems to work without it.....
    // vm.calculations(res);
  ...
  vm.parseFootprint = function (data) {
    ...
    // var projectOut = { userInfo: [], userType: [], dataIn: [] };
    ...
    // No we can't just bring them in...unless it's global.
    // projectOut.userInfo.push({ selectedMonth: user.selectedMonth }, { selectedYear: user.selectedYear }, { project: user.project });
    // projectOut.userType = sendData;
  ...
  // HMM still shows up as empty even if you pass it as an argument:
